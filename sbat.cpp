...
void gcta::gene_snpset(string gAnno_file, int wind)
{
    int i = 0, j = 0;
    int snp_count;

    // read SNP details from bim file global
    vector<string> snp_name(_include.size());
    vector<int> snp_chr(_include.size());
    vector<int> snp_bp(_include.size());

    for (int k = 0 ; k < _include.size() ; k++) {
        snp_name[k] = _snp_name[_include[k]];
        snp_chr[k] = _chr[_include[k]];
        snp_bp[k] = _bp[_include[k]];
    }
    ...
    for (i = 0; i < gene_num; i++) {
        ...
        if(skip){
            ...
            //write snpset
            rogoodsnp << gene_name[i] << endl;
            for (int k = 0; k < snp_name.size(); k++) rogoodsnp << snp_name[k] << endl;
            rogoodsnp << "END" << endl << endl;

        if((i + 1) % 100 == 0 || (i + 1) == gene_num) cout << i + 1 << " of " << gene_num << " genes written.\r";
    }
    rogoodsnp.close();
}
