...
void gcta::read_IRG_fnames(string snp_info_file, string fname_file, double GC_cutoff)
{
    ...
    if(!i_snp_info) LOGGER.e(0, "can not open the file ["+snp_info_file+"] to read.");
    int i=0,j=0;
    string str_buf;
    ...
    if(!omap) LOGGER.e(0, "can not open the file ["+map_file+"] to write.");
    ...
    if(!i_fnames) LOGGER.e(0, "can not open the file ["+fname_file+"] to read.");
    ...
    if(!oped) LOGGER.e(0, "can not open the file ["+ped_file+"] to read.");
...
void gcta::read_one_IRG(ofstream &oped, int ind, string IRG_fname, double GC_cutoff)
{
    ...
    if(!i_IRG) LOGGER.e(0, "can not open the file ["+IRG_fname+"] to read.");
    char a1='0', a2='0';
    ...
    if(snp_num!=_snp_num) LOGGER.e(0, "the number of SNPs specified in the summary file does not match that in the raw genotype file ["+IRG_fname+"].");
    for(i=0; i<6; i++) getline(i_IRG, str_buf);
    for(i=0; i<snp_num; i++){
        getline(i_IRG, str_buf);
        StrFunc::split_string(str_buf, vs_buf, "\t ,;\n");
        if(vs_buf[0]!=_snp_name[i]) LOGGER.e(0, "the SNP ["+vs_buf[0]+"] specified in the summary file does not match that in the raw genotype file ["+IRG_fname+"]. The order of the SNPs have been changed?");
        pid=vs_buf[1];
