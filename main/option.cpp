...
#error Only Windows, Mac and Linux are supported.
        ...
        } else if (strcmp(argv[i], "--global-heidi-thresh") == 0) {
            global_heidi_thresh = atof(argv[++i]);
            if(global_heidi_thresh <0 || global_heidi_thresh >1)
                LOGGER.e(0, "--global-heidi-thresh, Invalid p-value threshold for global HEIDI-outlier test.");
            LOGGER<<"--global-heidi-thresh "<<global_heidi_thresh<<endl;            
    ...
    if (!RG_fname_file.empty()) {
        ...
        if (hapmap_genet_dst) pter_gcta->genet_dst(bfile, hapmap_genet_dst_file);
            ...
            else if (mtcojo_flag) pter_gcta->mtcojo(mtcojolist_file, ref_ld_dirt, w_ld_dirt, freq_thresh, gwas_thresh, clump_wind_size, clump_r2_thresh, global_heidi_thresh, indi_heidi_thresh, ld_fdr_thresh, nsnp_gsmr);
