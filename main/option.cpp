...
#error Only Windows, Mac and Linux are supported.
    ...
    if((mtcojo_flag || gsmr_flag) && !gsmr_so_flag && !ref_ld_flag && !w_ld_flag) LOGGER.w(0, "The GSMR analysis will be performed assuming no sample overlap between the GWAS data for exposure and outcome.");
    // OpenMP
    stringstream ss;
    ss << thread_num;
#ifdef _WIN32
