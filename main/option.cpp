...
void option(int option_num, char* option_str[])
{
    ...
    int nsnp_gsmr=10, indi_heidi_mtd=0;
    double freq_thresh = 0.2, gwas_thresh=5e-8, global_heidi_thresh = 0.05, ld_fdr_thresh=0.05, clump_wind_size=10000, clump_r2_thresh=0.05;
...
#error Only Windows, Mac and Linux are supported.
        ...
        } else if(strcmp(argv[i], "--heidi-alg") == 0) {
            indi_heidi_mtd = atoi(argv[++i]);
            if(indi_heidi_mtd < 0 || indi_heidi_mtd > 2)
                LOGGER.e(0, "--heidi-alg should be 0, 1 or 2.");
            LOGGER<<"--heidi-alg "<<indi_heidi_mtd<<endl;
    ...
    if (!RG_fname_file.empty()) {
        ...
        if (hapmap_genet_dst) pter_gcta->genet_dst(bfile, hapmap_genet_dst_file);
            ...
            else if (gsmr_flag) pter_gcta->gsmr(gsmr_alg_flag, ref_ld_dirt, w_ld_dirt, freq_thresh, gwas_thresh, clump_wind_size, clump_r2_thresh, global_heidi_thresh, indi_heidi_mtd, ld_fdr_thresh, nsnp_gsmr, o_snp_instru_flag, gsmr_so_alg);
            else if (mtcojo_flag) pter_gcta->mtcojo(mtcojo_bxy_file, ref_ld_dirt, w_ld_dirt, freq_thresh, gwas_thresh, clump_wind_size, clump_r2_thresh, global_heidi_thresh, indi_heidi_mtd, ld_fdr_thresh, nsnp_gsmr);
